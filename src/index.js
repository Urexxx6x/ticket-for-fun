...
for (const file of events) {
	...
	log.console(log.format(`> Lucifer Bot Loaded &7${event.event}&f event`));
}
...
for (const file of commands) {
	...
	log.console(log.format(`> Lucifer Bot Loaded &7${config.prefix}${command.name}&f command`));
}

log.info(`Lucifer Bot Loaded ${events.length} events and ${commands.length} commands`);

const one_day = 1000 * 60 * 60 * 24;
const txt = '../user/getbacks/text';
const clean = () => {
	...
	for (const file of files) {
		...
		if (Math.floor(diff / one_day) > config.getbacks.text.keep_for) {
			fs.unlinkSync(join(__dirname, txt, file));
			total++;
		}
	}
	if (total > 0) log.info(`Deleted ${total} old text ${utils.plural('getback', total)}`);
};

if (config.getbacks.text.enabled) {
	clean();
...
process.on('unhandledRejection', error => {
	log.warn('Error 404');
	log.warn(`404 ${error.name}: ${error.message}`);
	log.error(error);
});
