...
module.exports.add = (message) => {
	...
	if (config.getbacks.text.enabled) { // text getbacks
		let path = `../../user/getbacks/text/${message.channel.id}.txt`,
	...
	if (config.getbacks.web.enabled) { // web archives
		let raw = `../../user/getbacks/raw/${message.channel.id}.log`,
			json = `../../user/getbacks/raw/entities/${message.channel.id}.json`;
...
module.exports.export = (Ticket, channel) => new Promise((resolve, reject) => {	
	...
	(async () => {
		...
		let raw = `../../user/getbacks/raw/${channel.id}.log`,
			json = `../../user/getbacks/raw/entities/${channel.id}.json`;

		if (!config.getbacks.web.enabled || !fs.existsSync(join(__dirname, raw)) || !fs.existsSync(join(__dirname, json))) return reject(false);
		...
		let endpoint = config.getbacks.web.server;
